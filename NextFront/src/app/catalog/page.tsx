...
import { useBasket } from '@/Hooks/useBasketHooks';

export default function Catalog() {
  const productHooks = useProducts();
  const basketHooks = useBasket();
              ...
              <div className="flex">
                ...
                <div className="mx-3">
                  ...
                  <div className="flex items-center mt-2">
                    <button onClick={() => basketHooks.Decrease(item.id)} className="text-gray-500 focus:outline-none focus:text-gray-600">
                      <i className="bx bx-minus h-5 w-5" />
                    </button>
                    <span className="text-gray-700 mx-2">{item.quantity}</span>
                    <button onClick={() => basketHooks.Increase(item.id)} className="text-gray-500 focus:outline-none focus:text-gray-600">
          ...
          {filteredProducts.map((product: Product) => (
            <div key={product.id} className="w-full max-w-sm mx-auto rounded-md shadow-md overflow-hidden">
              <div
                ...
                style={{ backgroundImage: `url('data:image/jpeg;base64,${product.imageUrl}')` }}
              >
                <button onClick={() => basketHooks.AddToBasket(product.id)} className="p-2 rounded-full bg-indigo-600 text-white mx-5 -mb-4 hover:bg-indigo-500 focus:outline-none focus:bg-indigo-500">
                  <svg
                    className="h-5 w-5"
                    fill="none"
                    strokeLinecap="round"
